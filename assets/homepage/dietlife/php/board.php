    ...
    <div>
        <table>
            ...
            <?php while ($row = mysqli_fetch_array($res2)) {
                $num = $row['boardnum'];
                $title = str_replace(">", "&gt", str_replace("<", "&lt", str_replace($row['boardtitle'], mb_substr($row['boardtitle'], 0, 40, "utf-8") . "...", $row['boardtitle'])));
            ...
            ?>
                <tr style="cursor:pointer;" onClick="location.href='/boardread.php?x=<?php echo $num; ?>'">
                    <th><?php echo $num2; ?></th>
                    <th><?php if (mb_strlen($row['boardtitle'], "utf-8") > 30) {
                            echo $title;
                        } else {
                            echo $title2;
                        } ?></th>
