...
$(function(){
	...
	$('#btnNext').click(function(){
		...
		$('.roundBtn img').attr("src","images/ico_slider.png"); //속성 값 변경	
		
		if(n==4){ //순환했을때 0번 속성이 바뀌지 않을때! 
			$(".roundBtn img:eq(0)").attr("src","images/ico_slider_on.png");	
			
		}
		$(".roundBtn img:eq("+(n-1)+")").attr("src","images/ico_slider_on.png");	
	...
	$('.roundBtn li').click(function(){
		...
		//눌렀을때 선택 그림으로 변환하는것 
		$('.roundBtn img').attr("src","images/ico_slider.png"); //초기값 지우기가 첫번째
		$('img',this).attr("src","images/ico_slider_on.png"); //선택 라운드 보이기가 그다음
	});
	...
	$(window).scroll(function(){
		...
		if($(this).scrollTop()>=800){
			...
			$('header').css("position","fixed").css("background","rgba(255,255,255,1)");
			
		}else{
			
			$('header').css("position","relative").css("background","rgba(255,255,255,0.7)");
